...
export class AdminDashboard implements OnInit {
  ...
  departmentForm = this.formBuilder.nonNullable.group({ name: ['', Validators.required], code: ['', [Validators.required, Validators.pattern(/^[A-Za-z]{2,8}$/)]] });
  instructorForm = this.formBuilder.nonNullable.group({ name: ['', Validators.required], employeeNumber: ['', [Validators.required, Validators.pattern(/^[A-Za-z0-9-]{2,12}$/)]], password: ['', [Validators.required, Validators.minLength(6)]], departmentId: ['', Validators.required] });
  ...
  submit(form: any, request: () => any) {
    ...
    this.busy = true;
    request().subscribe({ next: () => { this.message = 'Saved successfully.'; this.busy = false; form.reset(); this.loadData(); }, error: (error: any) => { this.showError(error); this.busy = false; } });
  }

  createDepartment() { this.submit(this.departmentForm, () => this.adminService.createDepartment(this.departmentForm.getRawValue())); }
  createInstructor() { this.submit(this.instructorForm, () => this.adminService.createInstructor(this.instructorForm.getRawValue())); }
  createStudent() { this.submit(this.studentForm, () => this.adminService.createStudent(this.studentForm.getRawValue())); }
  createCourse() { this.submit(this.courseForm, () => this.adminService.createCourse(this.courseForm.getRawValue())); }
  createSection() {
    const value = this.sectionForm.getRawValue();
    this.submit(this.sectionForm, () => this.adminService.createSection({ ...value, schedule: [{ day: value.day, slot: value.slot, room: value.room }] }));
  }

  reviewCourse(id: string, status: 'approved' | 'rejected') { this.adminService.reviewCourseRequest(id, status).subscribe({ next: () => this.loadData(), error: error => this.showError(error) }); }
  reviewSection(id: string, status: 'approved' | 'rejected') { this.adminService.reviewEnrollment(id, status).subscribe({ next: () => this.loadData(), error: error => this.showError(error) }); }
